...
def CNN1D(X_train, X_test, y_train, y_test):
    ...
    #     X_train = X_train.to_numpy().reshape(X.iloc[train].shape[0], X.iloc[train].shape[1], 1)
    ...
    #     # model.summary()
    #
    #     model.fit(X_train, y_train, epochs=2, validation_split=0.1, shuffle=False)
    ...
    # model.fit(X_train, y_train, epochs=2, validation_split=0.1, shuffle=False, verbose=0)
    model.fit(X_train, y_train, epochs=2, validation_split=0.1, shuffle=False, verbose=1)
    prediction = model.predict_classes(X_test)
    print("ACC",100 * accuracy_score(y_test, prediction))
    print("RMSE:", mean_squared_error(y_test, prediction,squared=False))
    print("MAE:", mean_absolute_error(y_test, prediction))
...
def CNN2D(X_train, X_test, y_train, y_test):
    ...
    nX = X_train.to_numpy()  # X_train to 2D - array
    ...
    model.fit(m, y_train, validation_split=0.1, epochs=2, shuffle=False, verbose=1)
...
def LR(X_train, X_test, y_train, y_test):
    ...
    model = LogisticRegression(C=10, max_iter=1000, n_jobs=-1, solver='lbfgs')
...
def RF(X_train, X_test, y_train, y_test):
    ...
    model = RandomForestClassifier(n_estimators=20, min_samples_leaf=1, max_features='auto',  # max_depth=10,
...
# X_train, X_test, y_train, y_test = train_test_split(Xo, yo['FLAG'], test_size=0.1, random_state=0)
...
# LRBM(X_train, X_test, y_train, y_test)
